...
model.fit([x1_train, x2_train],[y1_train, y2_train], epochs=100, batch_size=1,
        validation_data=([x1_val, x2_val],[y1_val, y2_val])) # validation_data=머신한테 니가 검증해가면서 학습하라는 뜻
...
mse = model.evaluate([x1_test, x2_test], [y1_test, y2_test], batch_size=1) # evaluate함수에 x_test, y_test값을 넣으면 loss a[0], acc a[1] 값이 나온다.
print("mse : ", mse[0])
print("mse : ", mse[1])
print("mse : ", mse[2])
print("mse : ", mse[3])
