...
y1y2y3 = np.append(y1y2, y3, axis=1)
print(y1y2y3.shape)

from sklearn.model_selection import train_test_split
x1_train, x1_test, y1y2y3_train, y1y2y3_test = train_test_split(
    x1, y1y2y3, random_state=33, test_size=0.4, shuffle=False
) # x 값을 x_train, x_test로 분류 y 값을 y_train, y_test로 분류
x1_val, x1_test, y1y2y3_val, y1y2y3_test = train_test_split(
    x1_test, y1y2y3_test, random_state=33, test_size=0.5, shuffle=False # val 20% test 20% train 60%
...
model = Model(inputs = [input1,input2], 
              outputs = output1) # 어디서부터 어디까지 model이라는 걸 선언
model.summary() # 요약
...
model.fit([x1_train, x2_train],y1y2y3_train, epochs=100, batch_size=1,
        validation_data=([x1_val, x2_val],y1y2y3_val)) # validation_data=머신한테 니가 검증해가면서 학습하라는 뜻
...
mse = model.evaluate([x1_test, x2_test], y1y2y3_test, batch_size=1) # evaluate함수에 x_test, y_test값을 넣으면 loss a[0], acc a[1] 값이 나온다.
print("mse : ", mse[0])
print("mse : ", mse[1])

y1y2y3_predict = model.predict([x1_test,x2_test])
print(y1y2y3_predict)
...
RMSE = RMSE(y1y2y3_test, y1y2y3_predict)
print("RMSE: ", RMSE)

# R2 구하기 R2는 1이 나오면 잘한거 0이 나오면 최악이라는 지표일 뿐 0.9999 잘 됬다는 것은 아니다.
from sklearn.metrics import r2_score
r2_y1y2y3_predict = r2_score(y1y2y3_test, y1y2y3_predict)
print("R2 : ", r2_y1y2y3_predict)
...
model.fit([x1_train, x2_train],y1_train, epochs=100, batch_size=1,
        validation_data=([x1_val, x2_val],y1_val)) # validation_data=머신한테 니가 검증해가면서 학습하라는 뜻
# batch_size란 sample데이터 중 한번에 네트워크에 넘겨주는 데이터의 수를 말한다.
# 1:1 과외가 공부 잘되는 것 처럼 1억개 데이터이면 100만개씩 모아서 공부시키듯이
...
#4. 평가 예측
# mse = 평균 제곱 에러 오답에 가까울수록 큰 값이 나온다. 반대로 정답에 가까울수록 작은 값이 나온다.
mse = model.evaluate([x1_test, x2_test], y1_test, batch_size=1) # evaluate함수에 x_test, y_test값을 넣으면 loss a[0], acc a[1] 값이 나온다.
print("mse : ", mse[0])
print("mse : ", mse[1])
