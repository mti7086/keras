...
x1 = np.array([range(101, 201), range(311,411), range(100, 200)])
...
print(x1x2.shape) # (100,6)
print(y1y2.shape) # (100,6)


from sklearn.model_selection import train_test_split
x1x2_train, x1x2_test, y1y2_train, y1y2_test = train_test_split(
    x1x2, y1y2, random_state=50, test_size=0.4, shuffle=False
) # x 값을 x_train, x_test로 분류 y 값을 y_train, y_test로 분류
x1x2_val, x1x2_test, y1y2_val, y1y2_test = train_test_split(
    x1x2_test, y1y2_test, random_state=50, test_size=0.5, shuffle=False # val 20% test 20% train 60%
...
x1x2 = Dense(20, activation='relu')(input1) # dense1 layer/ input: 1, ouput:5
x1x2 = Dense(15)(x1x2) # dense2 layer/ input: 5, ouput:3
x1x2 = Dense(10)(x1x2) # dense3 layer/ input: 3, ouput:4
x1x2 = Dense(8)(x1x2)
x1x2 = Dense(8)(x1x2)
x1x2 = Dense(8)(x1x2)
x1x2 = Dense(8)(x1x2)
...
model = Model(inputs = input1, outputs = y1y2) # 어디서부터 어디까지 model이라는 걸 선언
model.summary() # 요약
...
model.fit(x1x2_train,y1y2_train, epochs=200, batch_size=3,
        validation_data=(x1x2_val,y1y2_val)) # validation_data=머신한테 니가 검증해가면서 학습하라는 뜻
...
mse = model.evaluate(x1x2_test, y1y2_test, batch_size=1) # evaluate함수에 x_test, y_test값을 넣으면 loss a[0], acc a[1] 값이 나온다.
print("mse : ", mse[0])
print("mse : ", mse[1])

y_predict = model.predict(x1x2_test)
print(y_predict)

# RMSE 구하기 RMSE = MSE에 루트를 씌워준 것
from sklearn.metrics import mean_squared_error
def RMSE(y1y2_test, y_predict):
    return np.sqrt(mean_squared_error(y1y2_test, y_predict)) # mse = mean squared error/sqrt=루트값
    # y_predict와 주어진 값 y_test와 비교
print("RMSE: ", RMSE(y1y2_test, y_predict))

# R2 구하기 R2는 1이 나오면 잘한거 0이 나오면 최악이라는 지표일 뿐 0.9999 잘 됬다는 것은 아니다.
from sklearn.metrics import r2_score
r2_y_predict = r2_score(y1y2_test, y_predict)
