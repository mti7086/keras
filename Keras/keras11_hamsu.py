...
# input1 = Input(shape=(1, )) # 최초의 input/ 컬럼 = 열 = 1개
...
input1 = Input(shape=(1, )) # 최초의 input/ 컬럼 = 열 = 1개
xx = Dense(5, activation='relu')(input1) # dense1 layer/ input: 1, ouput:5
...
output1 = Dense(1)(xx) # output1 layer/ input: 4, ouput:1

model = Model(inputs = input1, outputs = output1) # 어디서부터 어디까지 model이라는 걸 선언
model.summary() # 요약
